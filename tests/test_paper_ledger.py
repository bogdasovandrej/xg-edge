...
def _only_key(document: dict, section: str) -> str:
    keys = list(document[section])
    assert len(keys) == 1
    return keys[0]
...
def test_enrollment_is_one_per_match_and_idempotent() -> None:
    ...
    candidate_id = _only_key(updated, "enrollments")
    assert updated["enrollments"][candidate_id]["fixture_id"] == "m1"
    actions = updated["enrollments"][candidate_id]["strategy_actions"]
...
def test_diverse_candidates_for_same_match_are_enrolled_separately() -> None:
    ledger = new_paper_ledger(created_at=T0 - timedelta(hours=1))
    ...
    assert operation["enrolled"] == 2
    assert len(updated["enrollments"]) == 2
    assert {row["market_cluster"] for row in updated["enrollments"].values()} == {
        "HOME_RESULT", "DRAW_RESULT"
    }
...
def test_official_result_settles_all_paper_bets_with_valid_pinnacle_clv() -> None:
    ...
    settlement = settled["settlements"][_only_key(settled, "settlements")]
    assert settlement["closing_benchmark"] == "pinnacle_fair_1x2"
    assert settlement["closing_odds"] == pytest.approx(1 / .55)
...
def test_result_settles_but_diagnostic_close_never_becomes_clv() -> None:
    ...
    assert settled["settlements"][_only_key(settled, "settlements")]["closing_odds"] is None
...
def test_explicit_official_result_map_works_without_claiming_clv() -> None:
    ...
    assert settled["settlements"][_only_key(settled, "settlements")]["outcome"] == "away"
    assert settled["settlements"][_only_key(settled, "settlements")]["closing_odds"] is None
...
def test_v11_multi_market_ledger_migrates_without_changing_events(tmp_path) -> None:
    opened, _ = update_paper_ledger(
        new_paper_ledger(created_at=T0 - timedelta(hours=1)),
        _payload(_candidate()),
        now=T0,
    )
    legacy = deepcopy(opened)
    events_before = deepcopy(legacy["strategies"])
    candidate_id = _only_key(legacy, "enrollments")
    enrollment = legacy["enrollments"].pop(candidate_id)
    for field in (
        "candidate_id", "market_family", "market_cluster", "market_taxonomy_version"
    ):
        enrollment.pop(field)
    legacy["enrollments"] = {"m1": enrollment}
    legacy["schema_version"] = "paper-trading-ledger/1.1"
    legacy["policy"].pop("maximum_candidates_per_match")
    legacy["policy"].pop("diversity_taxonomy")
    legacy["policy"]["one_candidate_per_match"] = True
    path = tmp_path / "legacy.json"
    path.write_text(__import__("json").dumps(legacy), encoding="utf-8")

    migrated = load_paper_ledger(path)
    assert migrated["schema_version"] == "paper-trading-ledger/1.2"
    assert migrated["strategies"] == events_before
    migrated_id = _only_key(migrated, "enrollments")
    assert migrated["enrollments"][migrated_id]["fixture_id"] == "m1"
...
def test_goal_market_enrollment_and_automatic_settlement(
    ...
    assert operation["enrolled"] == 1
    enrollment_id = _only_key(opened, "enrollments")
    assert opened["enrollments"][enrollment_id]["market"] == market
    ...
    assert operation["settled"] == 1
    assert settled["settlements"][enrollment_id]["selection_result"] == expected
