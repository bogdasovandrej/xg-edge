...
Обновлено: 2026-08-18. Ветка: `codex/uefa-workflow-v2` от актуального
`origin/main`.
...
1. Зафиксировать логические commits, push, PR/merge и дождаться CI.
2. Запустить live/data workflow, затем проверить GitHub Pages.
3. Опубликовать тот же проверенный source через Sites.
