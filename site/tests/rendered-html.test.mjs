...
test("server-renders the xg-edge live dashboard", async () => {
  ...
  assert.match(html, /FULL LINE/);
  assert.match(html, /Фоновый CLV-аудит/);
  ...
  assert.doesNotMatch(html, /Текущий вердикт|общий гейт закрыт|CLV пока не измерен/);
...
test("uses the public snapshot and contains no disposable starter", async () => {
  ...
  assert.match(page, /Фоновый CLV-аудит/);
  assert.match(page, /fair не маскируется под доступную цену/);
  ...
  assert.match(page, /Нет подтверждённых коэффициентов/);
  assert.match(page, /model_market_forecasts/);
  assert.match(page, /Расчётная линия вероятностей/);
  assert.match(page, /НЕ КОТИРОВКА БУКМЕКЕРА/);
  ...
  assert.match(page, /INITIAL_FORECAST_LIMIT = 20/);
  assert.match(page, /visibleForecasts\.map/);
  assert.match(page, /Показать ещё/);
  assert.match(page, /PAPER BANKROLL/);
  assert.match(page, /PaperTradingLab/);
  assert.match(page, /payload\.paper_trading/);
  assert.match(page, /Пустые карточки ROI скрыты/);
  ...
  assert.match(page, /Прогноз \+ вся линия/);
  assert.match(page, /Full-line Brier/);
  assert.match(page, /Top-3 model/);
