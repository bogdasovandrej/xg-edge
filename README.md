...
**https://bogdasovandrej.github.io/xg-edge/weekly.html**. Он ранжирует будущие
матчи по срочности разбора, качеству досье, свежести подтверждённой линии и
сценарной хрупкости. Балл 0–100 — не вероятность ставки. Снимки коэффициентов
старше шести часов, снятые до прогноза или не прошедшие строгую проверку, дают
нулевой рыночный вклад; PAPER-кандидаты повторно валидируются в браузере и
никогда не разрешают исполнение реальными деньгами.

Новый CLV-эксперимент описан в
[`docs/prospective-clv-v2.md`](docs/prospective-clv-v2.md). Он хранится в
отдельном `prospective-clv/2.0` журнале, использует LCB95 вместо простого
point-edge и не смешивает новые Betfair Exchange наблюдения со старым
Pinnacle-протоколом. Контур остаётся PAPER_ONLY, пока не подключён платный sharp
feed, не появился `market-residual-v1` для Top-5 и не завершены primary +
replication когорты.

Версия 0.8 держит отдельный контур будущих матчей и не смешивает его с
...
- экспериментальная модель квалификаций ЛЧ, ЛЕ и ЛК: ClubElo + Poisson, отдельная
...
- по каждому матчу рассчитываются 40 score-resolvable исходов: 1X2, двойной
  шанс, DNB, ОЗ, тоталы, индивидуальные тоталы и азиатские форы;
- три разнородных модельных сценария показываются сразу с консервативной
  поправкой надёжности 3–6 п.п.; CLV-аудит идёт в фоне и не скрывает прогноз;
- без проверенной котировки сценарий остаётся `MODEL FORECAST`, а не выдаётся
  за bookmaker-value ставку;
...
- архив будущих прогнозов замораживает всю линию до kickoff и после результата
  считает попадания и Brier по каждому сохранённому рынку;
...
- официальный settlement FIFA/UEFA отдельно обновляет результат за 90 минут,
  log-loss и Brier;
- PAPER-журнал автоматически отбирает и рассчитывает не только 1X2, но также
  тоталы, ОЗ, азиатские форы, индивидуальные тоталы, двойной шанс и DNB, если
  поставщик вернул полную свежую цену и модель прошла строгий фильтр;
- карточки, угловые, четвертные азиатские линии и проход дальше остаются
  закрытыми до появления проверяемого результата/правила расчёта; отсутствие
  данных не подменяется выдуманной ставкой;
- всё выполняется в GitHub Actions: ноутбук может быть выключен.

### Ворота value и бесплатный источник эджа

Все ворота считает один модуль `src/xgedge/decision/pricing.py`:
`fair = 1 + L/W` (не `1/p` — на рынке с возвратом это разные числа),
`min_entry = fair × 1.08`, `value_pct = (W·(odds−1) − L) / (W+L) × 100`.
Порог — `value_pct ≥ 8`. Экспертный балл `value_rating` не является воротами
и не участвует ни в одной сортировке: это только справочная колонка.

Четыре инварианта закреплены тестами в CI (`tests/test_golden_invariants.py`):
целая линия обязана иметь `push > 0` и режим `WPL_FULL`; состояния всегда
дают в сумме 1; конфликт источников счёта возвращает `BLOCKED` без расчёта;
и ставка с высоким рейтингом, но низким `value_pct`, обязана не пройти.

`src/xgedge/decision/market_consensus.py` даёт эдж без платных данных и без
нейросети: справедливая вероятность для конторы строится по **остальным**
конторам (leave-one-out), поэтому книга не голосует сама за себя. Расхождение
с консенсусом — кандидат на разбор, а не доказанный эдж. Меньше трёх книг —
явный отказ, а не уверенный ноль.

### Хранилище, скринер и журнал

`src/xgedge/storage/db.py` — SQLite прямо в репозитории. Это безопасно,
потому что все пишущие workflow уже делят concurrency-группу
`xgedge-data-writers`, то есть писатель всегда один. История цен
append-only: без неё нет ни дельты для скринера, ни CLV. Записи `analysis`
не перезаписываются, а вытесняются через `superseded_by`.

`scripts/record_quotes.py` дописывает каждый снимок цен и считает движение:
`DRIFT / STEAM / LARGE_MOVE (≥7%) / FROZEN`. Крупное движение помечается
`NEEDS_MECHANISM` и намеренно не считается ни хорошим, ни плохим сигналом.
`LIMIT_SIGNAL` возвращается только когда провайдер сам сообщил об изменении
лимита — из цены его выводить нельзя.

`scripts/update_journal.py` хранит `fair` на момент входа отдельным полем:
пересчёт задним числом измерил бы текущую модель, а не принятое тогда решение.

### Покрытие турниров

`src/xgedge/data/coverage.py` объявляет 16 турниров: топ-5 лиг, пять
национальных кубков, ЛЧ/ЛЕ/ЛК (квалификация, группа, плей-офф), РПЛ и ЧМ.
Каждый показывает свой источник и статус `ACTIVE` / `MISSING_KEY`, поэтому
пустой турнир читается как отсутствие ключа, а не как отсутствие матчей.

ЛЧ/ЛЕ/ЛК и ЧМ идут из официальных фидов UEFA/FIFA — ключей не требуют.
Топ-5 лиг — football-data.org. РПЛ и кубки — api-football (`API_FOOTBALL_KEY`),
чей бесплатный план даёт 100 запросов в сутки при потребности в семь.
...
15 минут автоматически получает доступные линии Bet365 и Unibet, сопоставляет
...
1X2, тоталы, «обе забьют» и азиатские форы (когда выбранные букмекеры возвращают
эти рынки). Подтверждающий CLV gate по-прежнему использует только de-vigged
Pinnacle 1X2: остальные линии показаны отдельно как `EXPERIMENTAL_SHADOW`, пока
каждая из них не пройдёт свой prospective-протокол.

Текущий каталог Odds-API.io не принимает `Pinnacle`, хотя это название всё ещё
встречается в старых примерах документации. Поэтому адаптер запрашивает только
две проверенные активные книги (`Bet365`, `Unibet`) — это лимит подключённого
плана. Их closing consensus остаётся диагностическим и не подменяет
подтверждающий Pinnacle CLV.
...
В раскрытом разборе матча сначала показаны последние пять официальных матчей
обеих команд (товарищеские исключены), В/Н/П, соперник, счёт и Elo. `npxG`
показывается только при наличии shot/event-level источника; `npxG*` затем
применяет проверяемые поправки на красные карточки и силу соперника. Если UEFA
даёт результат, но не даёт события ударов и xG, интерфейс честно пишет
`нет event-data`: счёт и общие удары не используются как поддельный xG.
