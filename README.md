...
Версия 0.5 добавляет отдельный контур будущих матчей и не смешивает его с
...
- официальный The Odds API сохраняет только pre-match 1X2 snapshots: discovery
  не чаще раза в сутки и closing-window каждые 15 минут;
...
Код и облачный workflow уже подключены, но секретов в репозитории нет. Получите
API key на [официальной странице The Odds API](https://the-odds-api.com/#get-access)
и добавьте его в GitHub: `Settings → Secrets and variables → Actions → New
repository secret`, имя — `THE_ODDS_API_KEY`. Не вставляйте ключ в issue,
commit, JSON или чат. После этого market monitor запускается автоматически;
без ключа он безопасно пропускает сетевые запросы.

Free-план на момент релиза даёт 500 credits в месяц. Поэтому production-запрос
ограничен рынком `h2h`, discovery имеет 24-часовой cooldown, а quota headers
сохраняются для контроля бюджета. Totals будет включён только после отдельного
протокола оценки, иначе он удвоил бы расход без подтверждающей метрики.
...
228 тестов работают без сети. CI проверяет Python 3.10 и 3.12.
