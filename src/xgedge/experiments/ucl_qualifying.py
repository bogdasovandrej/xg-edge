...
def normalize_team_name(name: str) -> str:
    ...
    asciiish = asciiish.casefold().translate(str.maketrans({
        "æ": "ae",
        "ð": "d",
        "ł": "l",
        "ø": "o",
...
def add_uefa_elo_fallbacks(
    ...
    *,
    ...
    alias resolves. Missing clubs receive leakage-safe UEFA result updates on
    the same scale: known historical opponents are anchored to ClubElo and an
    otherwise unknown club starts at the lower quartile of the supplied
    ClubElo population. This avoids mixing an arbitrary 1500 UEFA scale with
    real ClubElo values near 900-1900.
    """
    ...
    finite_ratings = [
        float(rating.elo) for rating in ratings if math.isfinite(float(rating.elo))
    ]
    fallback_prior = (
        float(np.quantile(finite_ratings, 0.25)) if finite_ratings else 1500.0
    )
    ...
    for match in source_matches:
        ...
        for side in ("home", "away"):
            team_id = str(match.get(f"{side}_id") or "").strip()
            known = index.lookup(str(match.get(side) or ""))
            if team_id and known is not None:
                priors[("club", team_id)] = known.elo
    official_elo = PointInTimeElo(
        source_matches,
        priors=priors,
        config=EloConfig(initial_rating=fallback_prior),
    )
    output = list(ratings)
    ...
    summary = {
        ...
        "fallback_prior_elo": round(fallback_prior),
    }
